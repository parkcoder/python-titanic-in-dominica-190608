...
if __name__ == '__main__':
    ...
    def random_batch(data, size):
        ...
        for i in random_index:
            random_inputs.append(data[i][0]) # target 행렬
            random_labels.append([data[i][1]]) # 행렬을 리스트에 담다

        return random_inputs, random_labels
    # learning option
    training_epoch = 300
    learning_rate = 0.1
    batch_size = 20 # 한번에 학습할 데이터 큭;
    ...
    print("embeding_size %d"%(embedding_size))
    ...
    with tf.Session() as sess:
        ...
        for step in range(1, training_epoch + 1):
            ...
            if step % 10 == 0:
                print('loss at step : %d loss val : %d' % (step, loss_val))
